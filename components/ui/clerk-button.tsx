...
const clerkButtonVariants = cva(
  ...
  [
    ...
    "text-left font-medium transition duration-300 ease-[cubic-bezier(0.4,0.36,0,1)]",
    "rounded-md",
    ...
    // Base sizing
    "text-sm h-[1.875rem] px-3 ring-1",
  ],
  {
    variants: {
      variant: {
...
const ClerkButton = React.forwardRef<HTMLButtonElement, ClerkButtonProps>(
  ({ className, variant, size, ...props }, ref) => {
    return (
      <button
        className={cn(clerkButtonVariants({ variant, size, className }))}
        ref={ref}
        {...props}
      />
    );
...
export function ClerkButtonDemo() {
  return (
    <div className="p-8 space-y-4 bg-gray-100">
      <h2 className="text-xl font-bold">Clerk-Style 3D Buttons</h2>
