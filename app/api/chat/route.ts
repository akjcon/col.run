...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userId) {
      console.log(
        "Using quick response for unauthenticated user"
      );
      const response = await quickChatResponse(
        lastMessage.content
      );
    ...
    if (!userData) {
      console.log(
        "User data not found, using quick response"
      );
      const response = await quickChatResponse(
        lastMessage.content
      );
    ...
    const useFullContext = shouldUseFullContext(
      lastMessage.content
    );
