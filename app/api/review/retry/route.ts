 ...
 * Body: { planId: string }
 * Resets a failed plan to "generating" and re-runs generation with the same params.
 */
export async function POST(request: Request) {
  try {
    ...
    if (!raceGoal) {
      return NextResponse.json({ error: "Plan missing race data" }, { status: 400 });
    }

    // Use stored athlete profile, or build a reasonable default from what we have
    const athleteProfile: AthleteProfile = data.athleteProfile || {
    ...
    // Reset status
    await db.collection("generatedPlans").doc(planId).update({
      status: "generating",
      error: null,
    });
    ...
    after(async () => {
      try {
        ...
        console.log(`[Review Retry] Plan complete: ${planId} (${(generationTimeMs / 1000).toFixed(1)}s)`);
      } catch (err) {
        console.error(`[Review Retry] Failed:`, err);
        await db.collection("generatedPlans").doc(planId).update({
          status: "error",
