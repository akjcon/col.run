...
export function calculateCurrentWeek(
  ...
  const daysFromMonday = startDayOfWeek === 0 ? 6 : startDayOfWeek - 1; // Convert to Monday-based
  ...
  const clampedWeek = Math.max(1, Math.min(totalWeeks, weekNumber));
...
export function calculateWorkoutDates(
  ...
  firstWeekStart.setDate(startDateObj.getDate() - daysFromMonday);
  ...
  return weeks.map((week) => ({
    ...
    workouts: week.workouts.map((workout) => {
      ...
      weekStart.setDate(firstWeekStart.getDate() + (week.weekNumber - 1) * 7);
